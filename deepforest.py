...
def roc_whole():
    ...
    classifiers = [SVC(C=0.1, kernel='poly', degree=1, gamma=0.01, coef0=0, class_weight='balanced', probability=True),
                   SVC(kernel='rbf', C=0.1, class_weight='balanced', gamma=1, probability=True),
                   SVC(C=1, coef0=0, kernel='poly', degree=1, gamma=10, class_weight={0: 1, 1: 1}, probability=True),
                   SVC(C=0.1, kernel='poly', degree=1, gamma=0.01, coef0=0, class_weight='balanced', probability=True),
                   SVC(kernel='poly', C=0.1, class_weight={0: 1, 1: 1}, gamma=10, degree=2, coef0=10,
                       probability=True),
                   SVC(C=1, coef0=0, kernel='poly', degree=1, gamma=10, class_weight={0: 1, 1: 1},
                       probability=True)]
...
def roc_df():
    ...
    for file in files:
        ...
        plt.subplot(2, 3, index + 1)
        for train, test in cv.split(X, y):
            # clf = gcForest(tolerance=0.0, min_samples_cascade=20)
            ...
            clf = RandomForestClassifier(101)
            clf.fit(X[train], y[train])
            probas_ = clf.predict_proba(X[test])
...
def test():
    ...
    df = pd.read_csv('./data/clf_CN.csv')

    y = df.pop('DECLINED').values
    # X = MinMaxScaler().fit_transform(df.values)
    X = df.values

    for train, test in StratifiedKFold(n_splits=2, random_state=0).split(X, y):
        gc = GCForest(config)
        gc.fit_transform(X[train], y[train])
        r = gc.predict_proba(X[test])
        print('shape=', r.shape)
        print(r)
    # r=gc.predict_proba(X)
